...
async function readBoundedMethodsJson(response: Response): Promise<unknown> {
  ...
  if (Number.isFinite(declaredLength) && declaredLength > MAX_METHODS_RESPONSE_BYTES) {
    throw new RazorpayTestProxyError(502, 'RAZORPAY_TEST_INVALID_METHODS', 'Razorpay returned an oversized payment-method response.');
  }
  ...
  try {
    ...
    throw new RazorpayTestProxyError(502, 'RAZORPAY_TEST_INVALID_METHODS', 'Razorpay returned invalid payment-method JSON.');
  }
}
...
export function normalizeRazorpayTestMethods(value: unknown): RazorpayTestMethods {
  if (!value || typeof value !== 'object' || Array.isArray(value)) {
    throw new RazorpayTestProxyError(502, 'RAZORPAY_TEST_INVALID_METHODS', 'Razorpay returned invalid payment methods.');
  }
  ...
  if (!payload.netbanking || typeof payload.netbanking !== 'object' || Array.isArray(payload.netbanking)) {
    throw new RazorpayTestProxyError(502, 'RAZORPAY_TEST_INVALID_METHODS', 'Razorpay returned invalid netbanking methods.');
  }
...
export class RazorpayTestClient {
  private methodsCache?: { expiresAt: number; value: RazorpayTestMethods };
  ...
  async getMethods(): Promise<RazorpayTestMethods> {
    ...
    try {
      ...
      throw new RazorpayTestProxyError(502, 'RAZORPAY_TEST_METHODS_UNAVAILABLE', 'Razorpay payment methods are temporarily unavailable.');
    }
    const value = await readBoundedMethodsJson(response);
    if (!response.ok) throw upstreamError(value, response.status);
    const normalized = normalizeRazorpayTestMethods(value);
