...
export function usePaymentStatus(
  id: string,
): PaymentStatusState & { refresh: () => Promise<void> } {
  ...
  useEffect(() => {
    ...
    const fetchLatest = async (): Promise<PublicPayment | undefined> => {
      ...
      setState((current) => ({
        ...current,
        refreshing: Boolean(current.payment),
        error: undefined,
      }));
      try {
        const fresh = await getPayment(id, currentController.signal);
        if (disposed || currentController.signal.aborted) return undefined;
        if (fresh.status === "pending") savePaymentSession(fresh);
        else clearPaymentSession(id);
        setState({ payment: fresh, loading: false, refreshing: false });
        return fresh;
      } catch (error) {
        if (disposed || currentController.signal.aborted) return undefined;
        const message =
    ...
    const poll = async () => {
      ...
      if (
        !disposed &&
        retryable &&
        (!payment || !isTerminalStatus(payment.status))
      )
        schedule();
    };
    const onVisibility = () => {
