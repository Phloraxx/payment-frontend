...
export function RazorpayLivePage() {
      ...
      try {
        ...
        setError(
          requestError instanceof ClientApiError
            ...
            : "Unable to load the Razorpay live order.",
        );
      } finally {
        setLoading(false);
      }
  ...
  useEffect(() => {
    ...
    let active = true;
    void discoverRazorpayLiveMethods(order.keyId)
      .then((enabledMethods) => {
        if (!active) return;
  ...
  const verify = useCallback(
    async (response: VerifyRazorpayLiveRequest) => {
      if (!order) return;
      setMessage(
      ...
      try {
        ...
        setError(
          requestError instanceof ClientApiError
            ? requestError.message
            : "Could not verify the Razorpay live payment.",
        );
      } finally {
        setProcessing(false);
      }
  ...
  const startPayment = () => {
    ...
    try {
      ...
      razorpay.createPayment(
        buildNetbankingTestPayment(order, bank.code, "live"),
      );
    } catch (paymentError) {
      setProcessing(false);
      setMessage(undefined);
  ...
  if (loading) {
    return (
      <CenteredState
        ...
        title="Loading live order"
        description="Checking the isolated Razorpay Live service."
      />
    );
  }
  if (!order) {
    return (
      <CenteredState
        ...
        title="Live order unavailable"
        description={error || "The Razorpay live order could not be found."}
      />
    );
  }
    ...
    <PageShell brandVariant="razorpay-live">
      <div className="relative z-10">
        <div className="rounded-2xl border border-sky-200 bg-sky-50 px-4 py-3 text-sm leading-relaxed text-sky-800">
          <strong>Razorpay Live Mode:</strong> this is a real ₹1 payment. Your
          bank account will be charged if you complete it.
        </div>
        ...
        {order.status === "created" && (
          <section className="mt-7 rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
            ...
            <p className="mt-4 text-center text-xs leading-relaxed text-slate-400">
              Razorpay’s payment-method popup is removed. Its secure bank page
              handles the real ₹1 authorization.
            </p>
            {sdkState === "loading" && (
        ...
        {captured && (
          ...
          >
            <strong>Live payment captured.</strong> Razorpay and the server both
            confirmed the real payment.
          </div>
        )}
        {failed && (
          <div
          ...
          >
            <strong>Live payment failed.</strong>{" "}
            {order.error || "Create another live order to retry."}
          </div>
        )}
        {message && !captured && (
          <div
