...
import type { RazorpayLiveMethods, RazorpayLiveOrder, VerifyRazorpayLiveRequest } from '../../shared/razorpay-live.js';
import { isRazorpayLiveTerminal } from '../../shared/razorpay-live.js';
import { PageShell } from '../components/PageShell';
import { ClientApiError, getRazorpayLiveMethods, getRazorpayLiveOrder, verifyRazorpayLiveOrder } from '../lib/api.js';
import {
...
export function RazorpayLivePage() {
  const { id = '' } = useParams();
  const [order, setOrder] = useState<RazorpayLiveOrder>();
  const [loading, setLoading] = useState(true);
  const [sdkState, setSdkState] = useState<SdkState>('loading');
  const [methods, setMethods] = useState<RazorpayLiveMethods>();
  ...
  const refresh = useCallback(async (signal?: AbortSignal) => {
    if (!id) return;
    try {
      const value = await getRazorpayLiveOrder(id, signal);
      setOrder(value);
    } catch (requestError) {
      if (requestError instanceof DOMException && requestError.name === 'AbortError') return;
      setError(requestError instanceof ClientApiError ? requestError.message : 'Unable to load the Razorpay live order.');
    } finally {
      setLoading(false);
    }
  }, [id]);
  ...
  useEffect(() => {
    let active = true;
    void Promise.all([loadRazorpayCustomSdk(), getRazorpayLiveMethods()])
      .then(([, enabledMethods]) => {
        if (!active) return;
  ...
  useEffect(() => {
    if (!order || isRazorpayLiveTerminal(order.status)) return;
    const timer = window.setInterval(() => void refresh(), 2_500);
    return () => window.clearInterval(timer);
  }, [order, refresh]);

  const verify = useCallback(async (response: VerifyRazorpayLiveRequest) => {
    if (!order) return;
    setMessage('Payment response received. Verifying securely with Razorpay…');
    setError(undefined);
    try {
      const updated = await verifyRazorpayLiveOrder(order.id, response);
      setOrder(updated);
      setMessage(updated.status === 'captured'
        ? 'Live payment captured successfully.'
        : 'Signature verified. Waiting for provider capture confirmation.');
    } catch (requestError) {
      setError(requestError instanceof ClientApiError ? requestError.message : 'Could not verify the Razorpay live payment.');
    } finally {
      setProcessing(false);
    }
  }, [order]);
  ...
  const startPayment = () => {
    ...
    setProcessing(true);
    setMessage(`Opening ${bank.name} secure Live bank authentication…`);
    setError(undefined);
    try {
      const razorpay = new window.Razorpay({
        key: order.keyId,
        redirect: true,
        callback_url: buildRazorpayCallbackUrl(order.id, window.location.origin, 'live'),
      });
      razorpay.on('payment.success', (response) => void verify(response));
      razorpay.on('payment.error', handlePaymentError);
      razorpay.createPayment(buildNetbankingTestPayment(order, bank.code, 'live'));
    } catch (paymentError) {
      setProcessing(false);
      setMessage(undefined);
  ...
  if (loading) {
    return <CenteredState icon={<CircleNotch className="h-10 w-10 animate-spin" />} title="Loading live order" description="Checking the isolated Razorpay Live service." />;
  }
  if (!order) {
    return <CenteredState icon={<WarningCircle className="h-10 w-10" />} title="Live order unavailable" description={error || 'The Razorpay live order could not be found.'} />;
  }
    ...
    <PageShell brandVariant="razorpay-live">
      <div className="relative z-10">
        <div className="rounded-2xl border border-sky-200 bg-sky-50 px-4 py-3 text-sm leading-relaxed text-sky-800">
          <strong>Razorpay Live Mode:</strong> this is a real ₹1 payment. Your bank account will be charged if you complete it.
        </div>
        ...
        <div className="mt-7 text-center">
          ...
          <p className="mt-5 text-xs font-bold uppercase tracking-[0.18em] text-slate-400">IEEE Razorpay Live Checkout</p>
        ...
        {order.status === 'created' && (
          <section className="mt-7 rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
            <div className="flex items-start gap-3">
              ...
              <div>
                <h3 className="font-semibold text-slate-950">Choose your bank</h3>
            ...
            <div className="mt-5">
              <label htmlFor="razorpay-live-bank" className="mb-2 block text-sm font-semibold text-slate-700">
                Bank
              </label>
              <select
                id="razorpay-live-bank"
                value={selectedBank}
            ...
            <p className="mt-4 text-center text-xs leading-relaxed text-slate-400">
              Razorpay’s payment-method popup is removed. Its secure bank page handles the real ₹1 authorization.
            </p>
        ...
        {captured && (
          <div role="status" className="mt-7 rounded-2xl border border-emerald-200 bg-emerald-50 px-5 py-4 text-center text-sm leading-relaxed text-emerald-800">
            <strong>Live payment captured.</strong> Razorpay and the server both confirmed the real payment.
          </div>
        )}
        {failed && (
          <div role="alert" className="mt-7 rounded-2xl border border-red-200 bg-red-50 px-5 py-4 text-center text-sm leading-relaxed text-red-700">
            <strong>Live payment failed.</strong> {order.error || 'Create another live order to retry.'}
          </div>
        )}
