...
const CREATE_DRAFT_KEY = "paygate:create-draft:v4";
...
export function loadPaymentSession(id: string): PublicPayment | undefined {
  ...
  try {
    ...
    if (
      ...
      !isPublicPayment(parsed.payment)
    ) {
      safeRemove(key(id));
      return undefined;
    }
...
function getOrCreateDraftRequestId(
  storageKey: string,
  amount: number,
  name?: string,
  externalId?: string,
...
export function getOrCreateRequestId(
  ...
  return getOrCreateDraftRequestId(CREATE_DRAFT_KEY, amount, name, externalId);
}
